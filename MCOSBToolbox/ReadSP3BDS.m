...
%------------------------------------------------------------------------------------------------------
function [sate] = ReadSP3BDS(ipath,DOYth,sate_mark,year)
    ...
    for i=1:len
        ...
        [~, ~, BDS1] = read_precise_eph(pr_obs);
        [~, ~, BDS2] = read_precise_eph(cu_obs);
        [~, ~, BDS3] = read_precise_eph(nx_obs);
        bdsx1 = BDS1.X(:,1:46); bdsy1 = BDS1.Y(:,1:46); bdsz1 = BDS1.Z(:,1:46);
        bdsx2 = BDS2.X(:,1:46); bdsy2 = BDS2.Y(:,1:46); bdsz2 = BDS2.Z(:,1:46);
        bdsx3 = BDS3.X(:,1:46); bdsy3 = BDS3.Y(:,1:46); bdsz3 = BDS3.Z(:,1:46);
        ...
        bdsl=size(sate.bdsx,2);
        if bdsl<bdssate
            sate.bdsx(:,bdsl+1:bdssate)=0;
            sate.bdsy(:,bdsl+1:bdssate)=0;
            sate.bdsz(:,bdsl+1:bdssate)=0;
        end
        break;
    end
end
